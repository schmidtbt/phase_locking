classdef PLVM < handle
    ...
    methods (Static)
        ...
        function output = resting_siPLV_whole_brain_func(seed_data,lh_clean_data,rh_clean_data,Fs,freqs, lh_meg_data,rh_meg_data)
            ...
            for j = freqs
                tic
                seed_ROI_complex_values=conj(plv.traces2Phases_normalized((seed_data),j,Fs,width));
                [1 toc]

                lh_complex_values=plv.traces2Phases_normalized((lh_clean_data),j,Fs,width);
                [2 toc]
                ...
                [5 toc]
                rh_complex_values=plv.traces2Phases_normalized((rh_clean_data),j,Fs,width);
                [6 toc]
        ...
        function output = resting_plv_pairewise_multiple_freq( seed_data, reference_data, Fs, Freqs, width )
            ...
            for j = Freqs
                seed_complex_values     = conj(plv.traces2Phases_normalized((seed_data),j,Fs,width));
                ref_complex_values      = plv.traces2Phases_normalized((reference_data),j,Fs,width);
        ...
        function output = resting_plv_pairewise_multiple_freq_phases( seed_data, reference_data, Fs, Freqs, width )
            ...
            for j = Freqs
                ...
                output{j}               = diff_vectors;
            end
        end
        
        function output = resting_plv_pairwise_stacked( data, Fs, Freqs )
            output = plv.resting_plv_pairewise_multiple_freq( data(1,:), data(2,:), Fs, Freqs );
        end
        ...
        function procdata = preprocess_data( data, Fs, Freqs )
            ...
            for i=1:size(data,1)
                ...
                for j = Freqs
                   procdata{i}{j} = plv.traces2Phases_normalized((data(i,:)),j,Fs,width);
                end
        ...
        function procvertex = preprocess_single_vertex( data, Fs, Freqs )
            ...
            for j = Freqs
               procvertex{j} = plv.traces2Phases_normalized((data),j,Fs,width);
            end
...
%             
        ...
        function output = resting_plv_pairwise_complex_stacked( data, Fs, Freqs )
            output = plv.resting_plv_pairewise_multiple_freq_complex( data{1}, data{2}, Fs, Freqs );
        end
        ...
        function [PLF,timeVec,freqVec] = traces2Phases_normalized(S,freqVec,Fs,width)
            ...
            %    fprintf(1,'%d ',i); 
                %for j=1:length(freqVec)
                    B(i,:) = plv.phasevec(freqVec,detrend(S(i,:)),Fs,width);
                %end
